...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    const [profileRes, recipesRes, logsRes] = await Promise.all([
      ...
      Fetch(
        new Request(`${process.env.SERVER_URL}/api/logging/?date=${dateStr}`),
        session,
      ),
    ]);
  ...
  if (intent === "recipe") {
    ...
    if (!recipeId) {
      return data<ActionData>(
        { error: "Select a recipe first." },
        { status: 400 },
      );
    }
    if (Number.isNaN(servingsToLog) || servingsToLog <= 0) {
      return data<ActionData>(
        { error: "Servings to log must be greater than 0." },
        { status: 400 },
      );
    }
    ...
    if (!itemName) {
      return data<ActionData>(
        { error: "Select a food first." },
        { status: 400 },
      );
    }
    if (!fdcId) {
      return data<ActionData>(
        { error: "Pick one USDA food from the list." },
        { status: 400 },
      );
    }
...
export default function MealLoggingPage() {
  ...
  function getRecipeCalories(recipe: RecipeListItem) {
    const macros = getRecipeMacros(
      recipe as unknown as { macros?: RecipeMacroShape },
    );
            ...
            <CardContent className="space-y-3">
              {!recipes.length ? (
                ...
                <Form method="post" className="space-y-3">
                  ...
                  <div>
                    <Label htmlFor="recipe_search" className="py-1">
                      Recipe
                    </Label>
                    <Input
                      id="recipe_search"
                  ...
                  {selectedRecipeId ? null : (
                    <p className="text-xs text-muted-foreground">
                      Select one recipe from the list.
                    </p>
                  )}
                  <div>
                      ...
                      {suggestions.map((food) => (
                        <li key={food.fdcId}>
                          ...
                          >
                            ...
                            <p className="text-xs text-muted-foreground">
                              {food.macros.calories} kcal | P{" "}
                              {food.macros.protein}g | C {food.macros.carbs}g |
                              F {food.macros.fat}g
                            </p>
                          </button>
                        </li>
