...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    const res = await Fetch(
      new Request(
        `${process.env.SERVER_URL}/api/analytics/progress/?date=${date}`,
        {
          headers: {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen w-full max-w-4xl space-y-3 mx-auto my-10">
        ...
        <Button variant={"link"} onClick={() => navigate(-1)}>
          {" "}
          <ArrowLeft className="h-3" /> Go back{" "}
        </Button>
      </div>
    );
  }
    ...
    <section className="w-full h-screen max-w-4xl space-y-4 mx-auto py-10">
      ...
      <div className="grid grid-cols-3 gap-2">
        <CaloriesStat
          ...
          color={
            macrosResult.goal.calories > macrosResult.current.calories
              ? "var(--chart-2)"
              : "var(--chart-1)"
          }
        />
      ...
      <div className="space-y-2">
        ...
        <Link
          to={`/analytics/logging?mode=simple&date=${currentDate}`}
          className="flex items-center"
        >
          {" "}
          Go to meals <ArrowRight className="h-3" />{" "}
        </Link>
      </div>
    </section>
  );
