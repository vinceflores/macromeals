...
import { Form, Link, data, redirect, useActionData, useLoaderData, useNavigation, useFetcher } from "react-router";
...
type LoaderData = { profile: Profile; recipe: ExtendedRecipeDetail | null; error?: string };
...
function normalizeMacros(recipe: RecipeDetail) {
  ...
  const total = recipe.macros?.total ?? {
    calories: flat.calories ?? 0, protein: flat.protein ?? 0,
    carbs: flat.carbs ?? flat.carbohydrates ?? 0, fat: flat.fat ?? 0,
  };
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  if (!Number.isInteger(id) || id < 1)
    return data<LoaderData>({ profile: EMPTY_PROFILE, recipe: null, error: "Invalid recipe ID." }, { status: 400 });
  try {
    const [profileRes, recipeRes] = await Promise.all([
      Fetch(new Request(`${process.env.SERVER_URL}/api/accounts/profile/`), session),
      Fetch(new Request(`${process.env.SERVER_URL}/recipe/${id}/`, { headers: { "Content-Type": "application/json" } }), session),
    ]);
    ...
    return data<LoaderData>({ profile: EMPTY_PROFILE, recipe: null, error: message }, { status: 404 });
  }
}
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  if (!session.data.access) return redirect("/auth/login");
  const id = Number(params.id);
  if (!Number.isInteger(id) || id < 1) return data<ActionData>({ error: "Invalid recipe ID." }, { status: 400 });
  const form   = await request.formData();
  const intent = String(form.get("intent") ?? "");

  if (intent === "delete_recipe") {
    try {
      const response = await Fetch(new Request(`${process.env.SERVER_URL}/recipe/${id}/`, { method: "DELETE" }), session);
      if (!response.ok && response.status !== 204)
        return data<ActionData>({ error: `HTTP ${response.status}` }, { status: 400 });
  ...
  if (intent === "toggle_visibility") {
    try {
      const res = await Fetch(new Request(`${process.env.SERVER_URL}/recipe/${id}/visibility/`, { method: "PATCH" }), session);
      if (!res.ok) return data<ActionData>({ error: `HTTP ${res.status}` }, { status: 400 });
      const updated = await res.json();
      return data<ActionData>({ success: `Recipe is now ${VISIBILITY_LABELS[updated.visibility] ?? updated.visibility}.` });
    } catch (e) {
      return data<ActionData>({ error: e instanceof Error ? e.message : "Failed." }, { status: 400 });
    }
  }
...
export default function RecipeDetailRoute() {
  ...
  const displayVis    = pendingToggle ? CYCLE[(CYCLE.indexOf(currentVis) + 1) % CYCLE.length] : currentVis;
  ...
  return (
    <div className="mx-auto w-full max-w-3xl p-6">
      <Link to="/recipes?tab=saved" className="text-sm underline">Back to Recipes</Link>
        ...
        <>
          <div className="mt-4 flex items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2 shrink-0">
              ...
              <Link to={`/edit/recipe/${recipe.id}`} className="rounded border px-3 py-1.5 text-sm hover:bg-accent">
                Edit
              </Link>
          ...
          <section className="mt-6 rounded border p-4">
            ...
            <p className="mt-2 text-sm">
              Total: {macros.total.calories} kcal | P {macros.total.protein}g | C {macros.total.carbs}g | F {macros.total.fat}g
            </p>
            <p className="mt-1 text-sm">
              Per serving: {macros.perServing.calories} kcal | P {macros.perServing.protein}g | C {macros.perServing.carbs}g | F {macros.perServing.fat}g
            </p>
          </section>
        </>
      )}
