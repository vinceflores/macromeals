...
export default function RecipesRoute() {
  ...
  const activeTab: "create" | "saved" = searchParams.get("tab") === "saved" ? "saved" : "create"
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="mx-auto w-full max-w-5xl p-6">
        ...
        <div className="mt-6 flex gap-2">
          <button type="button" onClick={() => setActiveTab("create")} className={`rounded border px-3 py-2 text-sm ${activeTab === "create" ? "bg-zinc-900 text-white" : ""}`}>
            Create Recipe
          </button>
          <button type="button" onClick={() => setActiveTab("saved")} className={`rounded border px-3 py-2 text-sm ${activeTab === "saved" ? "bg-zinc-900 text-white" : ""}`}>
            Saved Recipes
          </button>
        </div>
        ...
        {activeTab === "create" ? (
          <section className="mt-4 rounded border p-4">
            ...
            {actionData?.success ? <p className="mt-3 text-sm text-green-700">{actionData.success}</p> : null}
          </section>
        ) : (
